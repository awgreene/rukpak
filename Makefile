...
$(LINUX_BINARIES):
	CGO_ENABLED=0 GOOS=linux go build -gcflags="all=-N -l" -tags $(GO_BUILD_TAGS) $(VERSION_FLAGS) -o $(BIN_DIR)/$@ ./cmd/$(notdir $@)

$(BINARIES):
	CGO_ENABLED=0 go build -gcflags="all=-N -l" -tags $(GO_BUILD_TAGS) $(VERSION_FLAGS) -o $(BIN_DIR)/$@ ./cmd/$@
